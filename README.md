...
Optional data-source and advanced signal-engine packages are split into extras
so the core development install stays small:

```bash
pip install -e ".[market-data]"     # Tushare, yfinance, AKShare, CCXT
pip install -e ".[signal-skills]"   # smartmoneyconcepts, pyharmonics
pip install -e ".[analytics]"       # DuckDB-backed workflows
```
